...
// Authors: Updated by Rebecca Mantione, orginal code from Chris Fietkiewicz and Albright Dwarka (HWS '21). Requires SwingGraphics.java.
// Recreates the default "Fiber" simulation from anctoolkit.com.
// Intracellular stimulus is constant and timing controls extracellular stimulus.
// A total of 10 nodes are simulated, but only nodes 0 - 4 are plotted.
...
public class AxonStarter {
	...
	// static double[] node = new double[2];
	static ArrayList<Double> apStartTimeList = new ArrayList<>();
	static ArrayList<Double> apEndTimeList = new ArrayList<>();
	static ArrayList<Double> nodeT = new ArrayList<>();
	static boolean cont = true;
	static boolean cont2 = true;
	static int pos = 0;
	static int count = 0;

	public static void main(String[] args) {
		int numNodes = 10; // Number of nodes in the simulation, will add myelin between each node
		double dt = 0.001; // Integration step size (msec)
		double duration = 100; // Duration of entire simulation (msec)
		double stimDuration = 0.1; // Duration of extracellular stimulus (msec)
		double delay = 34.0; // Start time of extracellular stimulus (msec)
		Axon axon = new Axon(numNodes, dt); // create axon

		// Experiment setup
		double intStimAmplitude = 10; // For internal (intracellular) stimulus
		double extStimAmplitude = -700; // For external (extracellular) stimulus
...
//		System.out.println(N);
		...
		Color[] color = { Color.BLUE, Color.GREEN, new Color(0, 148, 101), new Color(252, 127, 3), Color.BLACK,
				Color.BLUE, Color.GREEN, new Color(0, 148, 101), new Color(252, 127, 3), Color.BLACK, Color.BLUE,
				Color.GREEN, new Color(0, 148, 101), Color.BLUE, Color.GREEN, new Color(0, 148, 101),
				new Color(252, 127, 3), Color.BLACK, Color.BLUE, Color.GREEN, new Color(0, 148, 101),
				new Color(252, 127, 3), Color.BLACK, Color.BLUE, Color.BLUE, Color.GREEN, new Color(0, 148, 101),
				new Color(252, 127, 3), Color.BLACK, Color.BLUE, Color.GREEN, new Color(0, 148, 101),
				new Color(252, 127, 3), Color.BLACK, Color.BLUE };

		SwingGraphics grapher = new SwingGraphics(); // Create single Grapher for all lines
		for (int k = 0; k < 10; k++) {
			apStartTimeList.clear();
			apEndTimeList.clear();
			boolean apStarted = false;
			for (int i = 0; i < N; i++) {
				if (!apStarted && data[k][i] >= 0) {
					apStarted = true;
					apStartTimeList.add(t[i]);
//					System.out.println("AP Start Time: " + apStartTime + " ms");

				} else if (apStarted && data[k][i] <= 0) {
//					System.out.println("AP End Time: " + apEndTime + " ms");
					apEndTimeList.add(t[i]);
					apStarted = false; // reset if you want to detect multiple spikes

				}
			}

			for (int f = 0; f < apEndTimeList.size(); f++) {
				System.out.println(apEndTimeList.size() + " " + k);
				double[] time = new double[2];
				time[0] = apStartTimeList.get(f);
				time[1] = apEndTimeList.get(f);
				double[] node = new double[2];
				node[0] = k;
				node[1] = k;
				grapher.graph(time, node, color[k]);
			}
		}

		// Graph results
//		SwingGraphics grapher = new SwingGraphics(); // Create single Grapher for all lines
//		grapher.graph(t, data[0], Color.BLUE);
//		grapher.graph(t, data[1], Color.GREEN);
//		grapher.graph(t, data[2], new Color(0, 148, 101));
//		grapher.graph(t, data[3], new Color(252, 127, 3));
		grapher.display();
		...
		for (int i = 0; i < N; i++) {
			...
			for (int j = 0; j < axon.numCompartments; j++) {
				...
				if (i >= first && i <= (first + intStimDuration)) {
					...
					axon.getNode(j).calculate_dv(intStimulus, 0.0);
				}
			}
...
// Axon class: A group of NodeCompartments and MyelinCompartments connected in an alternating sequence.
class Axon {
	int intStimNodeIndex = 0; // Index of node that receives intracellular stimulus
	double electrodeX = 2; // x-coordinate of external electrode, relative to left-most node (millimeters)
	...
	double Rint = 10; // Internodal (axial) resistance (ohms)
